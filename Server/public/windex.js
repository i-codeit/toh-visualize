...
function registerEvents() {
    let selection = document.getElementById("selectBox");
    document.getElementById("submit").onclick = function () { main_1.onClickEvent(); };
    selection.onchange = function () {
        main_1.reDrawBlocks(parseInt(selection.options[selection.selectedIndex].value));
    };
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const ui_objects_1 = __webpack_require__(/*! ./ui-objects */ "./src/ui-objects.ts");
const physics_1 = __webpack_require__(/*! ./physics */ "./src/physics.ts");
const eventQueue_1 = __webpack_require__(/*! ./eventQueue */ "./src/eventQueue.ts");
const axios_1 = __importDefault(__webpack_require__(/*! axios */ "./node_modules/axios/index.js"));
const constants_1 = __webpack_require__(/*! ./constants */ "./src/constants.ts");
let ui = new ui_objects_1.UIObjects();
let eq = new eventQueue_1.EventQueue();
let physics = new physics_1.Physics(ui, eq);
let lastTime = 0;
function initialize() {
    initializeObjects();
    draw();
 ...
 */
function reDrawBlocks(numberOfBlocks) {
    ui.clearCanvas();
    ui.setNumberOfBlocksToDraw(numberOfBlocks);
    initializeObjects();
    draw();
}
exports.reDrawBlocks = reDrawBlocks;
/**
...
function onClickEvent() {
    return __awaiter(this, void 0, void 0, function* () {
		// http://127.0.0.1:3001/
        let url1 = `/toh?blocks=${ui.getNumberOfBlocksToDraw()}`;
        // let url1 = `http://172.31.5.219/:3001`;
        let actualObj = [];
        axios_1.default({
            method: 'get',
            url: url1
...
function createOuterBoundary() {
    ...
    ctx.beginPath();
    ctx.lineWidth = constants_1.Constants.BOUNDARY_LINEWIDTH;
...
class Physics {
    ...
    animate(deltaTime) {
        ...
        if (this.currentEvent.blockId !== -1
    ...
    moveBlock(sliderValue) {
        let ctx = this.ui.getContext();
        let toTowerIndex = this.getTowerIndex(this.currentEvent.towerTo);
        ...
        ctx.bezierCurveTo(currentPos.x - 20, currentPos.y - (0.3 * currentPos.y), finalPos.x + 20, currentPos.y - (0.3 * currentPos.y), finalPos.x, finalPos.y);
    ...
    adjustTowerHeight() {
        let toTowerIndex = this.getTowerIndex(this.currentEvent.towerTo);
        let fromTowerIndex = this.getTowerIndex(this.currentEvent.towerFrom);
        this.ui.getTowers()[fromTowerIndex].setTowerTopPosition(this.ui.getTowers()[fromTowerIndex].getTowerTopPosition().y + this.ui.getBlocks()[this.currentEvent.blockId].getRadiusY());
        this.ui.getTowers()[toTowerIndex].setTowerTopPosition(this.ui.getTowers()[toTowerIndex].getTowerTopPosition().y - this.ui.getBlocks()[this.currentEvent.blockId].getRadiusY());
    }
    getTowerIndex(towerId) {
        let towerIndex = -1;
        if (towerId == "A") {
            towerIndex = 0;
        }
    ...
    isEventUpdated() {
        if (this.currentEvent.blockId === this.eq.getCurrentEvent().blockId
...
class Tower {
    ...
    setTowerTopPosition(towerTopPosition) {
...
class UIObjects {
    constructor() {
        this.towers = Array();
        this.blocks = Array();
        this.canvas = document.getElementById("toh");
    ...
    initializeTowers() {
        ...
        for (let index = 0; index < constants_1.Constants.NUM_OF_TOWERS; index++) {
            ...
            let tower = new tower_1.Tower(this.ctx, constants_1.TowerId[index], currentPos);
    ...
    initializeBlocks() {
        ...
        let numberOfBlocks = this.getNumberOfBlocksToDraw();
        let currentHeight = this.canvas.clientHeight;
        for (let index = 0; index < numberOfBlocks; index++) {
            let block = new block_1.Block(this.ctx, index, { x: this.towers[0].getPosition().x, y: currentHeight }, constants_1.BlockColor[index]);
    ...
    getNumberOfBlocksToDraw() {
        return this.numOfBlocks;
    }
    setNumberOfBlocksToDraw(value) {
